	...
	function prepare_object_argument( $object, $depth = 1 ) {
		...
		foreach ( $object_to_check as $key => $value ) {
			...
			if ( $is_private ) {
				...
				$variable_visibility = '<span style="color: #92008d;font-style: italic;">private</span> ';
				$variable_visibility .= ( $is_own_property ? $class_name : $parent_class_name ) . ' ';
			}
			else if ( $is_protected ) {
				$key                 = str_replace( '*', '', $key );
				$variable_visibility = '<span style="color: #92008d;font-style: italic;">protected</span> ';
			}
			else {
				$variable_visibility = '<span style="color: #92008d;font-style: italic;">public</span> ';
			}

			$output .= PHP_EOL . str_repeat( $code_space, $depth ) . $variable_visibility . '[ ' . $key_additional . '<span style="color: #92008d;">' . $key . '</span>' . $key_additional . ' ] => ' . prepare_simple_argument( $value );
